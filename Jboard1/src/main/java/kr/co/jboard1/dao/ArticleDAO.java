...
import kr.co.jborad1.db.DBHelper;
import kr.co.jborad1.db.SQL;
...
public class ArticleDAO extends DBHelper{
	...
	public ArticleDTO selectArticle(String no) {
		...
		try {
			conn =getConnection();
			psmt = conn.prepareStatement(SQL.SELECT_ARTICLE_VIEW);
			psmt.setString(1, no);
	...
	public void updatetArticle(ArticleDTO dto) {
	...
	public void deleteArticle(String no) {
		...
		try {
			...
			psmt.setString(1, no);
			//2번째가 parent 지만 no값을 검색해서 지우기 때문에 no
			psmt.setString(2, no);
			psmt.executeUpdate();
	...
	public void updateAticleForCommentPlus(String no) {
		try {
			conn= getConnection();
			psmt = conn.prepareStatement(SQL.UPDATE_ARTICLE_FOR_COMMENTPLUS);
			psmt.setString(1, no);
			psmt.executeUpdate();
			Close();
		} catch (Exception e) {
	...
	public void updateAticleForCommentMinus(String no) {
		try {
			...
			conn= getConnection();
			psmt = conn.prepareStatement(SQL.UPDATE_ARTICLE_FOR_COMMENTMINUS);
			psmt.setString(1, no);
			psmt.executeUpdate();
			Close();
